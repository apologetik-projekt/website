...
export function AudioPlayer({ slug, defaultTime }: { slug: string, defaultTime?: number }) {
	...
	function play(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
	...
	function pause(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
		e.currentTarget.focus()
		audioRef.current?.pause()
		setPlaying(false)
	}

	function rewind(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
		e.currentTarget.focus()
		if (audioRef.current) {
      ...
      <div className="rounded-px bg-black text-gray-100 dark:bg-black dark dark:text-white text-sm overflow-hidden p-2 box-border flex flex-col">
				...
				<div className="w-full flex items-center gap-1.5">
					<button
						...
						className="bg-gray-600/70 hover:brightness-110 transition-all duration-200 active:opacity-90 rounded-px p-2 leading-6 appearance-none focus:outline-none md:focus:ring-1 focus:ring-[#FFFF00] focus:ring-offset-2 focus:ring-offset-black">
						{ isLoading ? Spinner : isPlaying ? PauseIcon : PlayIcon }
					</button>
					<button aria-label="15 Sekunden zurückspulen" id="rewind" onClick={rewind} className="bg-gray-600/70 hover:brightness-110 transition-all duration-200 active:opacity-90 rounded-px p-2 leading-6 appearance-none focus:outline-none md:focus:ring-1 focus:ring-[#FFFF00] focus:ring-offset-2 focus:ring-offset-black">
					...
					<button id="speed" onClick={setSpeed} className="bg-gray-600/70 hover:brightness-110 transition-all duration-200 active:opacity-90 h-[34px] w-14 text-center rounded-px px-2 appearance-none focus:outline-none md:focus:ring-1 focus:ring-[#FFFF00] focus:ring-offset-2 focus:ring-offset-black">
